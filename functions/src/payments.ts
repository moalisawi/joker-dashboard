...
export const onPaymentCreated = onDocumentCreated("payments/{paymentId}", async (event) => {
  ...
  const month     = (payment["date"] as string | undefined)?.slice(0, 7);
  const createdBy = payment["createdBy"] as string | undefined;

  if (!month) {
    logger.error("[payments] Missing date on payment", event.params.paymentId);
    return;
  }
  ...
  if (createdBy) {
    ...
    if (userSnap.exists && !isUserActive(userSnap.data()!)) {
      logger.warn("[payments] Payment created by inactive user:", createdBy);
    }
  }

  // Incrementally update monthly analytics document
  const ref = db.collection("monthlyAnalytics").doc(month);
  const amt = (payment["amountUSD"] as number) ?? 0;
  ...
  logger.info(`[payments] Updated monthlyAnalytics/${month} +$${amt}`);
});
