...
## ⚠️ اقرأ قبل لمس Firestore

**[`docs/SECURITY-HARDENING-2026-08.md`](./docs/SECURITY-HARDENING-2026-08.md)** — سجل جلسة
٢ أغسطس ٢٠٢٦: لماذا لم يُنشر `firestore.rules` طوال ثلاثة أشهر، ما الذي صار حيّاً، وكيف تتراجع.

ثلاث نقاط لا تُنسى:
1. **قواعد Firestore لا تدعم `if` داخل الدوال** — الدالة تعبير `return` واحد. المخالفة تُرفض
   بـ `INVALID_ARGUMENT` بلا أي تفصيل، وهذا ما عطّل النشر ثلاثة أشهر.
2. **الترتيب عند تغيير عزل الموظفين:** تعبئة البيانات ← الفهارس ← القواعد. عكسه يحجب كل
   المدفوعات عن كل موظف.
3. **اختبر بحساب موظف لا بحساب مالك** — المالك يرى كل شيء ولن يلاحظ أي فرق.
