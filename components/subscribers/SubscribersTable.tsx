...
export default function SubscribersTable({
  ...
  // Unique nutrition team names derived from data
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm">
      {/* Filters */}
      <div className="p-4 border-b border-slate-50 flex flex-wrap gap-2 sm:gap-3 items-center">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-slate-50 text-slate-500 text-xs font-semibold">
              <th className="hidden sm:table-cell px-4 py-3 text-right">#</th>
              <th className="hidden sm:table-cell px-4 py-3 text-right">التاريخ</th>
              <th className="px-4 py-3 text-right">الاسم</th>
              <th className="hidden md:table-cell px-4 py-3 text-right">الهاتف</th>
              <th className="px-4 py-3 text-right">الباقة</th>
              {canRev && <th className="hidden lg:table-cell px-4 py-3 text-right">المبلغ</th>}
              {canRev && <th className="hidden lg:table-cell px-4 py-3 text-right">المدفوع</th>}
              {canRev && <th className="hidden sm:table-cell px-4 py-3 text-right">الصافي</th>}
              <th className="hidden sm:table-cell px-4 py-3 text-right">الانتهاء</th>
              <th className="px-4 py-3 text-right">الحالة</th>
              <th className="hidden md:table-cell px-4 py-3 text-right">الموظف</th>
            ...
            {paged.map((s, i) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="hidden sm:table-cell px-4 py-3 text-slate-600 text-xs whitespace-nowrap">{formatDate(s.date)}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        ...
                        {/* Mobile: show expiry inline under name */}
                        <span className="sm:hidden text-[10px] text-slate-400">{formatDate(s.expiryDate)}</span>
                      </div>
                    </Link>
                  </td>
                  <td className="hidden md:table-cell px-4 py-3 text-slate-500 text-xs font-mono whitespace-nowrap" dir="ltr">
                    {s.dialCode} {s.phone}
                  </td>
                  ...
                  {canRev && (
                    <td className="hidden lg:table-cell px-4 py-3 text-slate-700 text-xs whitespace-nowrap">
                  ...
                  <td className="hidden sm:table-cell px-4 py-3 text-slate-600 text-xs whitespace-nowrap">{formatDate(s.expiryDate)}</td>
                  <td className="px-4 py-3">
                    ...
                    <div className="flex gap-1 flex-wrap">
                      ...
                      {can("canWithdraw") && s.subscriptionState !== "withdrawn" && s.daysRemaining > 0 && (
                        <button onClick={() => onPause(s)} title="إيقاف مؤقت"
                          className="hidden sm:inline-flex px-2 py-1 rounded bg-amber-50 text-amber-700 hover:bg-amber-100 text-xs transition">
                          ⏸
                        </button>
                      )}
                      {can("canWithdraw") && s.subscriptionState !== "withdrawn" && s.status === "نشط" && !s.freezeData?.isFrozen && (
                        <button onClick={() => onFreeze?.(s)} title="تجميد"
                          className="hidden sm:inline-flex px-2 py-1 rounded bg-blue-50 text-blue-700 hover:bg-blue-100 text-xs transition">
                          ❄️
                        </button>
                      )}
                      {can("canWithdraw") && s.freezeData?.isFrozen && (
                        <button onClick={() => onResume?.(s)} title="استئناف"
                          className="hidden sm:inline-flex px-2 py-1 rounded bg-green-50 text-green-700 hover:bg-green-100 text-xs transition">
                          ▶️
                        </button>
                      )}
                      {can("canWithdraw") && (
                        <button onClick={() => onWithdraw(s)} title="انسحاب"
                          className={`hidden sm:inline-flex px-2 py-1 rounded text-xs transition ${
                            s.subscriptionState === "withdrawn"
                      ...
                      {can("canDelete") && (
                        <button onClick={() => onDelete(s.id, s.name)} title="حذف"
                          className="hidden sm:inline-flex px-2 py-1 rounded bg-red-50 text-red-700 hover:bg-red-100 text-xs transition">
                          🗑
                        </button>
                      )}
                    </div>
      ...
      <div className="flex items-center justify-between px-4 py-3 border-t border-slate-100">
        <span className="text-xs text-slate-400">
          عرض {Math.min(pageSize, filtered.length)} من {filtered.length} نتيجة
        </span>
        <div className="flex items-center gap-1.5">
          <span className="text-xs text-slate-500 ml-1">عرض:</span>
          {[10, 20, 50, 100].map((n) => (
            <button
              key={n}
