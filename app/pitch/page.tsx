...
export default function PitchPage() {
  ...
  return (
    <div dir="rtl" style={{ fontFamily: "'Cairo',Inter,system-ui,sans-serif", background: "#F5F7FB", color: "#111827" }}>
      ...
      {/* ══════════════════════════════════════════════════════════════════
          §10 ISO CERTIFICATIONS — Trust & compliance
      ══════════════════════════════════════════════════════════════════ */}
      <section style={{ background: "#FFFFFF" }} className="py-24">
        <div className="max-w-[1060px] mx-auto px-6">
          <div className="text-center mb-14">
            <Badge color="#059669" bg="#ECFDF5" border="rgba(5,150,105,.20)">
              <Award size={13} /> شهادات معتمدة دولياً
            </Badge>
            <H2Light sub="نظام مبني على أعلى معايير الجودة والأمان المعترف بها عالمياً">
              موثوق. معتمد. مُدقَّق.
            </H2Light>
          </div>
            ...
            {[
              {
                ...
                year: "2015",
                title: "إدارة الجودة",
                desc: "نظام إدارة الجودة الأكثر شيوعاً عالمياً — يضمن أن كل عملية في النظام مبنية على معايير موثقة وقابلة للقياس والتحسين.",
                icon: <Settings size={26} />,
                c: "#5B5FEF",
                bg: "#EEF0FF",
                border: "rgba(91,95,239,.18)",
                points: ["عمليات موثقة ومُدققة", "تحسين مستمر مبني على بيانات", "رضا العميل في صميم كل قرار"],
              },
              {
                code: "ISO 27001",
                year: "2022",
                title: "أمن المعلومات",
                desc: "المعيار الذهبي العالمي لحماية المعلومات — يضمن سرية بيانات عملاءك وسلامتها وإتاحتها في أي وقت.",
                icon: <ShieldCheck size={26} />,
                c: "#059669",
                bg: "#ECFDF5",
                border: "rgba(5,150,105,.18)",
                points: ["تشفير البيانات بالكامل", "صلاحيات وصول صارمة ومُدارة", "استجابة احترافية لأي حادثة أمنية"],
              },
              {
                code: "ISO 27017",
                year: "",
                title: "أمن الخدمات السحابية",
                desc: "امتداد لـ ISO 27001 يُغطي بيئة السحابة تحديداً — يكمل ضمانات Firebase وGoogle Cloud بمتطلبات تشغيلية إضافية.",
                icon: <Database size={26} />,
                c: "#7C3AED",
                bg: "#F5F3FF",
                border: "rgba(124,58,237,.18)",
                points: ["حماية البيانات على السحابة", "عزل كامل بين بيانات العملاء", "التزام بمعايير GDPR والخصوصية"],
              },
            ].map((cert, i) => (
              <div
                key={i}
                className={`reveal reveal-d${i + 1} card-hover rounded-[24px] p-7 relative overflow-hidden`}
                style={{ background: "#FAFBFF", border: `1px solid ${cert.border}`, boxShadow: "0 4px 24px rgba(15,23,42,.05)" }}
              >
                {/* Watermark number */}
                <span style={{
                  position: "absolute", top: 12, left: 16,
                  fontWeight: 900, fontSize: 52,
                  color: `${cert.c}08`, lineHeight: 1,
                  letterSpacing: "-.04em", userSelect: "none",
                }}>
                  {cert.code.replace("ISO ", "")}
                </span>
                ...
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 18 }}>
                  <div style={{
                    ...
                    background: cert.bg, color: cert.c,
                ...
                <div style={{ marginBottom: 10 }}>
                  <span style={{
                    ...
                    color: cert.c, letterSpacing: ".04em",
                  }}>
                    {cert.code}{cert.year && `:${cert.year}`}
                  </span>
                  <h3 style={{ fontSize: 17, fontWeight: 800, color: "#111827", margin: "4px 0 0", letterSpacing: "-.018em" }}>
                    {cert.title}
                  </h3>
                </div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
                  {cert.points.map((p, j) => (
                    <div key={j} style={{ display: "flex", alignItems: "center", gap: 9 }}>
                      ...
                      <span style={{ fontSize: 12.5, color: "#374151", fontWeight: 500 }}>{p}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {/* Bottom trust bar */}
          <div className="reveal rounded-[22px] p-6"
            style={{ background: "linear-gradient(135deg,#F0FDF4 0%,#ECFDF5 100%)", border: "1px solid rgba(5,150,105,.15)" }}>
            <div className="grid md:grid-cols-4 gap-5 text-center">
              {[
                { icon: <ShieldCheck size={18} />, title: "بيانات مشفرة", sub: "AES-256 end-to-end" },
                { icon: <Lock size={18} />,        title: "وصول محكوم",   sub: "Role-based access control" },
                { icon: <Eye size={18} />,          title: "سجل كامل",    sub: "Audit log لكل عملية" },
                { icon: <Globe size={18} />,        title: "إتاحة عالية", sub: "99.9% SLA مضمونة" },
              ].map((item, i) => (
                <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
                  <div style={{
                    ...
                    display: "flex", alignItems: "center", justifyContent: "center",
                    boxShadow: "0 2px 8px rgba(5,150,105,.12)",
                  }}>
                    {item.icon}
                  </div>
